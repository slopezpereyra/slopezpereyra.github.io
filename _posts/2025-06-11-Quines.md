...
>
> - $\left\| w := s \right\| \sigma := [\sigma \mid w : s]$  
> - $\left\| \textbf{code}(s) \right\|\sigma := \big< s / \delta_\sigma, ~ \sigma \big>$

> **Ejemplo**  
> Si $\sigma ~u = \textbf{skip}$, entonces
>
> $$
> \left\| \textbf{code} (w := u; u; u := \textbf{fail})  \right\|\sigma =
...
$$
\left\| \mathcal{C} \right\| \gamma = \left\| \textbf{code}(u := u; u) \right\| \gamma = \left< u := p; p, \gamma \right>
$$
...
Entonces la ecuación (1) garantiza que $\left\| \mathcal{C} \right\|\gamma$ imprime el código que asigna el valor $p$ a $u$ y luego ejecuta $p$\[^1\].
...
\begin{align*}
\left\| e \right\| \sigma 
&= \left\| u := \mathcal{C}; \mathcal{C} \right\| \sigma \\\\
&= \left\| \mathcal{C} \right\|[\sigma \mid u : \mathcal{C}] \\\\
&= \left\| \textbf{code}(u := u; u) \right\|[\sigma \mid u : \mathcal{C}] \\\\
...
$$
\therefore \forall \sigma \in \Sigma: \pi_1^2 \left( \left\| e \right\|\sigma \right) = e
$$
