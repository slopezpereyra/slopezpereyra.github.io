...
Observe that the function above does is not $C(\tau)$ exactly but the sequence
...
So far, we have derived the firing rate $r$ directly from an empirical
distribution of spikes. Another approach is to model $r$ as a linear combination
(i.e. a weighted sum) of the stimulus $s(t)$ with some appropriate weight
factor $\mathcal{D}(t)$. Since the time-dimension is continuous, such sum is
the following integration:

$$
r_{est}(t) = \int_0^{\infty} \mathcal{D}(\tau) s(t - \tau) ~ d\tau
$$

To find the appropriate $\mathcal{D}$ we minimize the difference between this
estimation and the empirical firing rate averaged across the duration of the
trial: 

$$
E = \frac{1}{T} \int_0^{T}  \big( r_{est}(t) - r(t) \big)^2 
$$

$E$ is minimized by setting its derivative with respect to $\mathcal{D}$ to zero. The result is that $\mathcal{D}$ depends on $Q_{rs}$ and $Q_{ss}$. In particular, the equation yields 

$$
\int_0^{\infty} Q_{ss}(\tau - \tau')\mathcal{D}(\tau') ~ d\tau' = Q_{rs}(-\tau)

$$
This is why this estimation method is called *reverse correlation*, because $Q_{rs}$ is evaluated at $-\tau$. This equation can be solved most easily if the stimulus $s$ is white noise, so that $Q_{ss}(\tau) = \sigma_{s}^2\delta(\tau)$ , resulting in 

$$
\sigma_{s}^{2}\int_0^{\infty}\delta(\tau - \tau')\mathcal{D}(\tau') = \sigma_{s}^{2}\mathcal{D}(\tau)
...
$$
\mathcal{D}(\tau) = \frac{Q_{rs}(-\tau)}{\sigma_{s}^{2} } = \frac{\langle r \rangle C(\tau)}{\sigma_{s}^2}
$$

This is sufficient to produce an estimation of the firing rate, since all
quantities involved in $\mathcal{D}$ can be derived from the empirical
spike-train.
...
$\frac{T}{{\Delta t}} = M$. For a particular spike train $\{ t_{1}, \ldots, t_{n} \}$ 
to occur, two things must be satisfied: exactly $n$ spikes must occur, and those
$n$ spikes must be located at $n$ of the $M$ possible time-bins. There are of
course $n!$ equivalent arrangements. Hence we quite readily obtain that
...
connect. Particularly, if $M \to \infty$ we have 
$M - n \approx M = \frac{T}{\Delta t}$. Using this and letting $u = -r\Delta t$
we obtain
