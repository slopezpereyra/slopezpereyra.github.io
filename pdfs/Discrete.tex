...
\begin{definition}
    Un corte es un conjunto de vértices que tiene la fuente y no tiene el
    resumidero; o bien es un conjunto $S \subseteq V$ t.q. $s \in S \land t
    \not\in S$.
\end{definition}

\begin{lemma}
    Sea $f$ un flujo, $S$ un corte. Entonces $v(f) = f(S, \overline{S}) -
    f(\overline{S}, S)$.
\end{lemma}
...
\begin{theorem}[Max flow, min cut]
    ...
    existe un corte $\mathcal{S}$ (que siempre es minimal) tal que $v(f) =
    Cap(\mathcal{S})$.
\end{theorem}
