...
\begin{definition}
    ...
    \begin{align*}
        C(l) \neq C(\tilde{l}) \iff l \cap \tilde{l} = \emptyset
    \end{align*}
...
Sea $\tilde{ G } = G - E(M)$ (borramos los lados de $M$ pero no los extremos).
...
\begin{align*} d_{\tilde{ G }}(z) = d_G(z) - 1, \forall  z \end{align*}

Pero $G$ es regular, con $d_G(z) = \Delta$ y por lo tanto $d_{\tilde{ G }}(z) =
\Delta - 1$. Por lo tanto $\tilde{ G }$ es regular con $\Delta(\tilde{ G }) =
...
probaremos que $\exists G$ bipartito con $G \subseteq \tilde{ G }, \Delta(G) =
\Delta(\tilde{ G }), \delta(G) + 1 = \delta(\tilde{ G })$. Repitiendo esta
construcción muchas veces ($\tilde{ G }$, $\tilde{ \tilde{ G } }, \tilde{
\tilde{ \tilde{ G } } }, \ldots$) llegamos a un grafo tal que 

\begin{align*}
    \delta(\tilde{ \tilde{ \tilde{ \tilde{ G }} } }) = \Delta(\tilde{ \tilde{ \tilde{ \tilde{ G } } } })
\end{align*}
...
Sea $\tilde{ X } = X \cup Y^*, \tilde{ Y } = Y \cup X^*$. Construimos $\tilde{ G }$ con $V(\tilde{ G }) = \tilde{ X } \cup \tilde{ Y }$ y 

\begin{align*}
    E(\tilde{ G }) = E \cup  E^* \cup  \overbrace{\left\{ v v^* : d_G(v) < \Delta \right\} }^{:= F}
\end{align*}

Vemos que $E$ une vértices de $X$ con $Y$, $E^*$ une vértices de $Y^*$ con
$X^*$, y $F$ une vértices de $X$ con $X^*$ o $Y^*$ con $Y$. Es decir,
$E(\tilde{ G })$ une vértices de $\tilde{ X }$ con $\tilde{ Y }$ y entonces
$\tilde{ G }$ es bipartito.

Además, 

\begin{align*}
    d_{\tilde{ G }}(v) = \begin{cases}
        \Delta & d_G(v) = \Delta \\ 
        d_G(v) + 1 & d_G(v) < \Delta
    \end{cases} = d_{\tilde{ G }}(v^*)
\end{align*}

De esto se sigue que $\Delta(\tilde{ G }) = \Delta(G)$ y $\delta(\tilde{ G }) = \delta(G) + 1$.
