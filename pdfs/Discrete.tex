...
\begin{definition}
    ...
    \begin{quote}
        ...
        \item $\overrightarrow{x_i x_{i+1}} \in E$ y
            $f(\overrightarrow{x_i}{x_{i+1}}) < c(\overrightarrow{x_ix_{i+1}})$.
        \item $\overrightarrow{x_{i+1}x_i} \in E$ y
            $f(\overrightarrow{x_{i+1}x_i}) > 0$.
        \end{itemize}
    \end{quote}
...
\begin{align*}
    ...
    ~\textbf{do}\\
    & ~ ~ ~ ~ C.A. := \text{$f$-camino aumentante $sx_0 \ldots x_{r-1} t$}\\
    &~ ~ ~ ~ \epsilon_i := \begin{cases}
        c(\overrightarrow{x_ix_{i+1}}) - f(\overrightarrow{x_ix_{i+1}}) &
        \text{Primer caso del $f$-camino aumentante} \\ 
        f(\overrightarrow{x_{i+1}x_i}) &\text{Segundo caso del $f$-camino
        aumentante}
    \end{cases} &  \\
    &~ ~ ~ ~  \epsilon = \min \epsilon_i\\
    &~ ~ ~ ~ \textbf{for} ~ \overrightarrow{xy} \in C.A. ~ \textbf{do}\\ 
    &~ ~ ~ ~  ~ ~ ~ ~  f(\overrightarrow{x_i x_{i+1}}) := \begin{cases}
        f(\overrightarrow{\overrightarrow{xy}}) + \epsilon & \text{Si la arista es
        forward} \\ 
            f(\overrightarrow{xy}) - \epsilon & \text{Si la arista es
        backward} 
    \end{cases} \\
    &~ ~ ~ ~ \textbf{od} \\ 
    &\textbf{od}
...
La idea parece ser tomar un subgrafo de $G$ adecuado y demostrar que su número
cromático es $\delta$; luego la propiedad de que $G$ es crítico termina la
prueba.

En particular, tomemos $G = (V - \left\{ d \right\} )$
