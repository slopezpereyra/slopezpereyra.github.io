...
\textbf{(3)} Consideremos la bola derecha, notando que existe una simetría en
las fuerzas operando sobre cada bola. 
...
\begin{enumerate}
    \item $\vec{G} + \vec{T} + \vec{E} = \vec{0}$
    \item $m = 30\text{g}, \qquad \ell = 15\text{cm}, \qquad \theta = \ang{5}$
    \item $G = -mg \hat{j}$ 
    \item $T_x = -T \sin \theta, \quad T_y = T \cos \theta$
    \item $\vec{E} = E_x \hat{i} = E \hat{i}$ (por ser la bola derecha, y las
        cargas iguales, la fuerza eléctcrica va hacia la derecha.
\end{enumerate}

\textbf{Valores desconocidos}: $a$, la mitad de la distancia entre las dos
bolas; $\vec{E}$, la fuerza eléctrica.

\textbf{Valor a determinar:} $q$, la carga de las bolas. 

\textbf{Relación entre los datos conocidos y el valor a determinar}: Por ley de
Couloumb,

\begin{equation*}
    E = \frac{1}{4\pi \epsilon_0}\frac{q^2}{4a^2}
\end{equation*}

\textbf{Estrategia:} $(i)$ Usar $(1)$ para encontrar una expresión para $E$.
$(ii)$ Determinar $a$. $(i i i )$ Usar la ley de Couloumb con $q$ como única
incógnita.

\end{quote}
\normalsize

$(i)$ De los datos se sigue fácilmente que 

\begin{align*}
    (G_y + T_y) \hat{j} + (E_x + T_x)\hat{i} = 0
    \iff ~
    & \begin{cases}
        G_y + T_y = 0 \\ 
        E_x + T_x = 0
...
Despejando $T$ en la ecuación de arriba y sustituyendo en la de abajo, 

\begin{equation}
    E =  mg \tan \theta
\end{equation}

$(i i)$ Es fácil ver geométricamente que $a = L \sin \theta $.

$( i i i)$ Por la ley de Coulomb, sustuyendo los valores concoidos de $E$ y $a$,

\begin{align*}
    &mg \tan \theta  =  \frac{ \kappa q^2}{4\left( \ell \sin
    \theta \right)^2 } \\
    \iff ~ ~ ~ 
    & q = \pm \sqrt{ \frac{mg \tan \theta \times 4 \ell^2 \sin^2 \theta }{\kappa} } 
\end{align*}

donde estoy obviando la escritura de las unidades. Notar que el ejercicio pide
la \textit{magnitud} de la carga, y por ende podemos quedarnos con el valor
positivo de esta raiz. Usando la aproximación $\kappa \approx 8.99 \times
10^{-9}$ se puede estimar el valor de la raíz.

\pagebreak 

\textbf{(4)}

$(a)$ La distancia entre el punto central
$\textbf{p}$ y cualquiera de los vérticas es 

\begin{equation}
    d = \frac{ \sqrt{\frac{1}{4}^2 + \frac{1}{4}^2} }{2} \approx 0.178
\end{equation}

Notemos que una partícula de carga unitaria y positiva ubicada en el centro
\textbf{p} sería atraída por las cargas negativas de la derecha y repelida por
las cargas positivas de la izquierda. Más aún, la carga inferior izquierda
empujaría la partícula equidireccionalmente a la atracción de la carga superior
derecha, y lo mismo sucede entre las cargas superior izquierda e inferior
derecha (ver diagrama).

\begin{center}
\begin{tikzpicture}[
    scale=1.0,
    every node/.style={transform shape},
    positive_charge/.style={circle, fill=blue!70, inner sep=3pt, label={#1}},
    negative_charge/.style={circle, fill=red!70, inner sep=3pt, label={#1}},
    field_vector/.style={-{Stealth[length=3mm]}, thick},
    resultant_vector/.style={-{Stealth[length=4mm]}, ultra thick, green!50!black}
]
    % --- Definir coordenadas ---
    \def\halfside{2}
    \coordinate (C) at (0, 0); % Centro
    \coordinate (q1) at (-\halfside, \halfside);  % Arriba-izquierda (+)
    \coordinate (q2) at (-\halfside, -\halfside); % Abajo-izquierda (+)
    \coordinate (q3) at (\halfside, \halfside);   % Arriba-derecha (-)
    \coordinate (q4) at (\halfside, -\halfside);  % Abajo-derecha (-)

    % --- Dibujar Ejes ---
    \draw[->, gray, thin] (-3, 0) -- (3, 0) node[right] {$x$};
    \draw[->, gray, thin] (0, -3) -- (0, 3) node[above] {$y$};

    % --- Dibujar el Cuadrado ---
    \draw[thick, black] (q1) -- (q3) -- (q4) -- (q2) -- cycle;
    
    % --- Dibujar Diagonales (referencia) ---
    \draw[dashed, gray, thin] (q1) -- (q4);
    \draw[dashed, gray, thin] (q2) -- (q3);

    % --- Colocar las Cargas ---
    \node[positive_charge={above left:$+q_1$}] at (q1) {};
    \node[positive_charge={below left:$+q_2$}] at (q2) {};
    \node[negative_charge={above right:$-q_3$}] at (q3) {};
    \node[negative_charge={below right:$-q_4$}] at (q4) {};

    % --- Etiquetar Centro ---
    \node[cross, draw=black, scale=0.5, label=below left:{$P(\text{centro})$}] at (C) {};

    % --- Etiquetar Dimensiones ---
    \draw[<->] ($(q2)+(0, -0.3)$) -- ($(q4)+(0, -0.3)$) node[midway, below] {Lado = 0.25 m};

    % --- Vectores de Campo Eléctrico (Parte a) ---
    % E1 (de q1, +) apunta lejos de q1 (abajo-derecha)
    \draw[field_vector, blue!70] (C) -- (1, -1) node[below, xshift=2mm, scale=0.8] {$\vec{E}_1$};
    % E2 (de q2, +) apunta lejos de q2 (arriba-derecha)
    \draw[field_vector, blue!70] (C) -- (1, 1) node[above, xshift=2mm, scale=0.8] {$\vec{E}_2$};
    % E3 (de q3, -) apunta hacia q3 (arriba-derecha)
    \draw[field_vector, red!70] (C) -- (1.2, 1.2) node[above right, scale=0.8] {$\vec{E}_3$};
    % E4 (de q4, -) apunta hacia q4 (abajo-derecha)
    \draw[field_vector, red!70] (C) -- (1.2, -1.2) node[below right, scale=0.8] {$\vec{E}_4$};

    % --- Vector Resultante E_total ---
    % Todos los componentes 'y' se cancelan, todos los 'x' se suman.
    \draw[resultant_vector] (C) -- (2.2, 0) node[right] {$\vec{E}_{\text{total}}$};

\end{tikzpicture}
\end{center}

Let $\vec{r}_{q_i p}$ be the unitary vector pointing from $q_i$ to the center
\textbf{p}. The electric field will be the sum of the fields generated by each
charge:

\begin{align*}
    \frac{1}{2\pi \epsilon_0} \sum_{i=1}^4 \frac{q_i}{d^2} \vec{r}_{q_i p}
    &= \frac{1}{2\pi \epsilon_0 d^2 } 
    \left[ q_1 \vec{r}_{q_1 p} + q_2 \vec{r}_{q_2 p} - q_3 \vec{r}_{q_3 p} - q_4
    \vec{r}_{q_4 p}\right] \\ 
    &= \frac{1}{2\pi \epsilon_0 d^2 } 
    \left[ q ( \vec{r}_{q_1 p} + \vec{r}_{q_2 p}  )-  q (\vec{r}_{q_3 p} + 
    \vec{r}_{q_4 p})\right] \\ 
    &= \frac{q}{2\pi \epsilon_0 d^2 } 
    \left[ ( \vec{r}_{q_1 p} + \vec{r}_{q_2 p}  ) -  (\vec{r}_{q_3 p} + 
    \vec{r}_{q_4 p})\right] \\ 
\end{align*}


The sum of these vectors, with $\theta = \pi / 4$, is
...
\begin{equation*}
    \therefore  ~ \vec{E} = \frac{q \sqrt{2} }{\pi \epsilon_0 d^2} \hat{i}
\end{equation*}

$(b)$ Recordemos que el campo eléctrico es la fuerza experimentada por una
partícula $q$ en un punto dado \textit{normalizada}, i.e. dividida por su propia
magnitud. Por ende, habiendo determinado $\vec{E}$, sabemos que la fuerza
experimentada por una partícula de magnitud $q$ ubicada en el punto \textbf{p}
es 

\begin{align*}
    q\vec{E} = \frac{q^2 \sqrt{2} }{\pi \epsilon_0 d^2} ~ \hat{i}
\end{align*}

Es decir, la partícula se movería horizontalmente a la derecha con un vector de
fuerza de magnitud $q^2 \sqrt{2} / \pi \epsilon_0 d^2 $.

\pagebreak  

\begin{shaded}
    
% Usamos 'enumerate' para el número del problema.
% \setcounter{enumi}{4} hace que la lista comience en 5.
\begin{enumerate}[label=\textbf{\arabic*.}]
\setcounter{enumi}{4}
\item Las cargas $q_1$ y $q_2$ se ubican en el eje $x$ a distancias $a$ y $b$ del origen, como se muestra en la figura.

    % Lista anidada para las partes (a) y (b)
    \begin{enumerate}[label=(\alph*)]
    \item Encuentre el campo eléctrico $\vec{E}$ resultante en el punto $p$, que está sobre el eje $y$.
    \item Evalúe el campo eléctrico en el punto $p$ en el caso especial de que $|q_1| = |q_2|$ y $a = b$.
    \end{enumerate}
...
\begin{center}
\begin{tikzpicture}[scale=1.2]
% Definimos las coordenadas que usaremos
\coordinate (O) at (0,0);     % Origen
\coordinate (P) at (0,4);     % Punto p
\coordinate (Q1) at (-3,0);   % Carga q1
\coordinate (Q2) at (4,0);    % Carga q2

% Dibujar los ejes
\draw [->, >=Stealth] (-4.5,0) -- (5.5,0) node[right] {$x$};
\draw [->, >=Stealth] (0,-0.5) -- (0,5) node[above] {$y$};

% Marcas en el eje x para las posiciones
\draw (O) -- (0, -0.1);
\draw (Q1) -- (-3, -0.1);
\draw (Q2) -- (4, -0.1);
...
% Dibujar las cargas
% Carga q1 (positiva)
\node (q1_node) [circle, draw, thick] at (Q1) {};
\node at (Q1) {$+$};
\node [below=4pt] at (Q1) {$+q_1$};

% Carga q2 (negativa)
\node (q2_node) [circle, draw, thick] at (Q2) {};
\node at (Q2) {$-$};
\node [below=4pt] at (Q2) {$-q_2$};

% Etiquetar el punto p
\node [above=2pt] at (P) {$p$};

% Dibujar las líneas de distancia r1 y r2
\draw (q1_node) -- (P) node[midway, above left] {$r_1$};
\draw (q2_node) -- (P) node[midway, above right] {$r_2$};
\draw (O) -- (P); % Línea del eje y desde el origen hasta p

% Etiquetas de distancia 'a' y 'b'
\node [below] at (-1.5, -0.1) {$a$};
\node [below] at (2.0, -0.1) {$b$};

% Dibujar los ángulos
% Ángulo alfa en q1
\pic [draw, "$\alpha$", angle radius=2.0cm, angle_eccentricity=1.3] {angle = O--Q1--P};

% Ángulo beta en q2
\pic [draw, "$\beta$", angle radius=2.0cm, angle_eccentricity=2.3] {angle = P--Q2--O};

\end{tikzpicture}
\end{center}
\end{shaded}

$(a)$ Sea $y$ la altura del punto $p$, i.e. $y = r_1 \sin \alpha = r_2 \sin
\beta$. Notemos:


\begin{align*}
    \vec{r}_{1p} &= a \hat{i} + y \hat{j}  \\ 
    \vec{r}_{2p} &= -b \hat{i} + y \hat{j}
\end{align*}

Entonces los vectores \textit{unitarios} desde $q_1$ a $p$ y desde $q_2$ a $p$
son $\vec{r}_{1p} / r_1, \vec{r}_{2p} / r_2$ respectivamente. Entonces


\begin{align*}
    \vec{E_1} = \kappa \frac{q_1}{r_1^3} \vec{r}_{1p}, \qquad \vec{E_2} = \kappa
    \frac{-q_2}{r_2^3}\vec{r}_{2p}
\end{align*}
...
\begin{align*}
    \vec{E} 
    &= \vec{E_1} + \vec{E_2} \\ 
    &= \kappa \left( \frac{q_1}{r_1^3} \vec{r}_{1p} - \frac{q_2}{r_2^3}
    \vec{r}_{2p} \right) \\ 
    &= \kappa \left( \varphi_1 \vec{r}_{1p} - \varphi_2 \vec{r}_{2p} \right)  &
   \left\{\text{Con } \varphi_i = q_i / r_i^3 \right\}   \\ 
    &= \kappa \left( \left[ \varphi_1 a, \varphi_1 y \right] - \left[
    -\varphi_2 b, \varphi_2 y\right]   \right)  \\ 
    &= \kappa \left[ \varphi_1 a + \varphi_2 b, \varphi_1 y -
    \varphi_2y \right]  \\ 
    &= \kappa ( \varphi_1 a + \varphi_2 b)\hat{i} + \kappa y (\varphi_1 -
    \varphi_2) \hat{j}
\end{align*}

$(b)$ Si asumimos $\left| a_1 \right|  = \left| q_2 \right| $ y $a = b$, nos
queda obviamente $\alpha = \beta$ y $r_1 = r_2$ en consecuencia. Por lo tanto, 
$\varphi_1 = \varphi_2$, y por ende

\begin{align*}
    \vec{E} 
    &=  2\kappa a \varphi \hat{i}  \\
    &= 2\kappa a \left( \frac{q}{r^3} \right)  \hat{i} \\ 
    &= 2 \kappa a \frac{q}{( \sqrt{a^2 + y^2} )^3 } \hat{i} \\ 
    &= \frac{2\kappa a q}{(a^2 + y^2)^{\frac{3}{2}}} ~ \hat{i}
\end{align*}

Esto tiene sentido: las componentes verticales de las fuerzas ejercidas por
$q_1$ y $q_2$ se cancelan, pues son de igual magnitud pero opuesta dirección.
Sin embargo, las componentes horizontales se suman: tanto $q_1$ como $q_2$
llevan la partícula situada en $p$ "hacia la derecha".

\pagebreak 

\textbf{(6)} Datos del problema: 

\begin{itemize}
    \item $x = 5$cm 
    \item $q = 3.2 \times  10^{-19}$C. 
    \item $m = 6.68 \times 10^{-27}$g. 
    \item $t = 2 \times  10^{-6}$g.
\end{itemize}

Notemos que 

\begin{itemize}
    \item $\vec{F} = F \hat{i} = Eq \hat{i}$
    \item $\vec{F} = ma \hat{i}$
\end{itemize}

Por lo tanto $Eq = ma$.

Como el campo es uniforme la aceleración es constante. Para una
aceleración constante, podemos relacionar la distancia recorrida, el tiempo
transcurrido y la velocidad inicial:
...
con lo cual ya podemos determinar $E$ numéricamente.

$(b)$ El trabajo realizado será por def. 

\begin{align*}
    W = \vec{F} \cdot \vec{x} = F x = \frac{2mx^2}{t^2}
\end{align*}


$(c)$ La diferencia de potencial elétrico entre dos puntos es el trabajo
realizado por unidad de carga: 

\begin{equation*}
    \Delta V = \frac{W}{q} = \frac{2mx^2}{t^2q} = E x
\end{equation*}

\pagebreak 

\begin{shaded}
    \textbf{(7)} Un electrón entra en una región de campo eléctrico uniforme (vea la figura) con una velocidad $v = 3 \times 10^6 \text{ m/s}$ y $E = 200 \text{ N/C}$. La longitud horizontal de los platos es $L = 0.1 \text{ m}$ y su separación es $h = 1.5 \text{ cm}$.
\begin{enumerate}
    \item[(a)] Encuentre la aceleración del electrón mientras esta en el campo eléctrico.
    \item[(b)] Asuma que la posición vertical del electrón al entrar al campo es $y = 0$, ¿logra abandonar la región de campo eléctrico? En caso de abandonarlo calcule el tiempo en el cual lo hace, caso contrario calcule la posición en la cual impacta.
\end{enumerate}
\end{shaded}
...
\begin{itemize}
    \item $\vec{F} = \vec{a}m$, con $m = 9.109 \times 10^{-31}$kg la masa de un
        electrón.
    \item $\vec{F} = \vec{E} e$, con $e = -1.602\times 10^{-19}$ la carga de un electrón.
    \item $\vec{a} = a \hat{j}$ 
    \item $\vec{E} = E \hat{j}$
\end{itemize}
...
\begin{align*}
    ...
    &a = \frac{Ee}{m} \\
    \iff ~ ~ ~ 
    &a = - \frac{200 (1.602)}{9.109} \frac{10^{-19}}{10^{-31}} \left[ \frac{
            \text{N C}}{\text{C kg}} \right]  \\ 
    \iff ~ ~ ~ 
    & a = -35.1740037326 \cdot 10^{12} ~ \frac{\text{m}}{\text{s}^2}
\end{align*}

$(b)$ Para una aceleración constante vertical $a$, nos queda una velocidad
vertical $v_y(t) = at$ (asumiendo que la velocidad vertical en el tiempo cero es
cero) y una función de posición vertical $y(t) = y_0 + \frac{a}{2}t^2$, con $y_0
= 0$ de acuerdo al enunciado.

Si asumimos que la aceleración horizontal es constante (no hay otras fuerzas en
juego), la distancia recorrida se relaciona con la velocidad inicial y el tiempo
en que se la recorre:

\begin{equation*}
    L = v_{0x}t
\end{equation*}

$v_{0x}$ es conocida, y el tiempo en que la partícula recorre la distancia $L$
es 

\begin{equation*}
    t_{\text{horizontal}} = \frac{ 0.1 \text{m}}{3\times 10^{6} \text{m/s}} =
    \frac{1}{3} \cdot 10^{-7} \text{ s}
\end{equation*}
...
\begin{align*}
    y(t_{\text{horizontal}}) 
    &= \frac{a}{2} t_{\text{horizontal}}^2 \\ 
    &=\frac{ -17.5870018663  }{3^2} \cdot (10^{-7})^2 \cdot 10^{12} \cdot \left[
    \frac{m}{s^2} s^2\right] \\ 
    &= -1.95411131848 \cdot 10^{-2} ~ \text{m}\\ 
    &=-0.01954111318 ~ \text{m}\\ 
    &=-1.954111318 \text{ cm}
\end{align*}

Esto excede la longitud vertical $h$ entre las dos barras y por lo tanto el
electrón no llega a salir.

\pagebreak 

\begin{shaded}
    \textbf{(10)} Considere dos cargas puntuales $q$ y $3q$ ubicadas a una
    distancia $d = 1$m. Encuentre la posición de los puntos sobre el eje que une
    las cargas en donde $V = 0$ y donde $\left| \vec{E} \right| = 0$.
\end{shaded}

Como nos interesa los puntos sobre el eje que une las dos cargas, definimos un
sistema de coordenadas unidimensional con $x = 0$ siendo la posición de la carga
$q$ y $x = 1$ la posición de la carga $3q$. (Claramente $[x] = $ m). En un punto
arbitrario $x_0$, el voltaje es la suma de los voltajes respecto a cada carga:

\begin{align*}
    V_{x_0} 
    &= V_{1} + V_2 \\ 
    &= \frac{\kappa q}{\left| x_0 - 0 \right| } + \frac{\kappa 3q}{\left| x_0 -
    1\right| } \\ 
    &= \kappa q \left( \frac{1}{|x_0|} + \frac{3}{\left| x_0 - 1 \right| } \right) 
\end{align*}

Se nos pide determinar los valores de $x$ tales que el voltaje es cero. Por
ende, planteamos la ecuación 

\begin{align*}
    \frac{1}{\left| x_0 \right| } + \frac{3}{\left| x_0 - 1 \right| } = 0
\end{align*}

Como nos interesan los puntos entre las dos cargas, $x_0 \in (0, 1)$, y por ende
$\left| x_0 \right| = x_0, \left| x_0 - 1 \right| = 1 - x_0$, y la ecuación
resulta 

\begin{align*}
    \frac{1}{x_0} + \frac{3}{1 - x_0} = 0 
    \iff 
    &x_0 + \frac{1-x_0}{3} = 0 \\ 
    \iff ~ ~ ~ 
    & 3x_0 + 1 - x_0 = 0 \\ 
...
Esta solución no es válida porque $-1 / 2 \not\in (0, 1)$. Luego, no existe
ningún punto entre las dos cargas donde el voltaje sea nulo.

$(b)$ El campo eléctrico en un punto $x \in (0, 1)$ es 

\begin{align*}
    \vec{E}(x) 
    &= \frac{\kappa q}{r_1^2(x)} \vec{r_1(x)} - \frac{\kappa
    3q}{r_2^2(x)}\vec{r_2(x)}\\ 
    &= \frac{\kappa q}{x^2} r_2\vec{(x)} - \frac{\kappa 3q}{( 1-x )^2} r_2\vec{(x)}
\end{align*}

donde el primer término es positivo porque la carga de prueba es repelida por la
carga $q$ y el segundo término negativo porque la carga de prueba es repelida
por la carga $3q$. Tomando sólo la magnitud del campo,


\begin{align*}
    \left| \vec{E}(x) \right| = \kappa q \left( \frac{1}{x^2} - \frac{3}{( 1-x
    )^2} \right) 
\end{align*}
...
\begin{align*}
    &\frac{1}{x^2} - \frac{3}{(1-x)^2} = 0 \\ 
    \iff    ~ ~ ~ 
    & \frac{1}{x} - \frac{\sqrt{3} }{1- x} = 0 \\ 
    \iff ~ ~ ~ 
    &\sqrt{3} x - (1-x) = 0 \\ 
    \iff ~ ~ ~ 
    &\sqrt{3}  x -1 + x = 0 \\ 
