...
previous iteration. Now consider the expression of $f(r)$ as 
...
The expression in $(10)$ is the numerator in $(8)$, whereby we obtain via
substitution: 
...
Equation $(11)$ ensures that the error scales quadratically. Now we wish to
bound the error expression in $(11)$.

The continuity of $f', f''$ ensures that these functions reach their extreme
values in a closed and bounded interval around $r$. Take $\delta > 0$ to define
a neighbourhood of length $\delta$ around $r$. Define 
...
Here, $c(\delta)$ is the maximum value which $e_{n+1}$ can take if $\zeta_n,
x_n$ are assumed to belong to the neighbourhood, according to equation $(11)$.
...
\begin{equation}
    \frac{1}{2} \left| f''(\zeta_0) / f'(x_0) \right| \leq c(\delta)
\end{equation}

Therefore, using the recurrent definition of $e_{n+1}$ in $(11)$, 

\begin{align*}
    \left| x_1 - r \right|  
    &= \left| e_1 \right|  \\ 
    &= \left| \frac{1}{2} f''(\zeta_0) / f'(x_0) e_0^2 \right|  \\ 
    &\leq |e_0^2| c(\delta) \\ 
    &\leq |e_0| \varrho &\left\{ \left| e_0 \right| \leq \delta
    \right\}  \\ 
