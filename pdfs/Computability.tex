...
\begin{problem}
    ...
    \omega$ una función $\Sigma$-computable. Entonces existe $\mathcal{P} \in
    Pro^{\Sigma}$ tal que 

    \begin{align*}
        f(\vec{x}, \vec{\alpha}, \mathcal{P}) \thickapprox  \Phi_{\#}^{n, m} (\vec{x},
        \vec{\alpha}, \mathcal{P})
    \end{align*}
\end{problem}
...
\begin{quote}
    ...
    ¿Qué condiciones debe satisfacer el programa $\mathcal{P}$ deseado? Ejecutar
    $\mathcal{P}$ partiendo de $[\![ x_1, \ldots, x_n, \alpha_1, \ldots,
    \alpha_m ]\!]$ debe equivaler a ejecutar $f(\vec{x}, \vec{\alpha},
    \mathcal{P}) $. Sabemos que $f$ tiene un programa $\mathcal{F} \in
    Pro^{\Sigma}$ asociado; parece natural suponer que el $\mathcal{P}$ deseado
    debe ser de la forma $\mathcal{W}\mathcal{F}$ con $\mathcal{W} \in
    Pro^{\Sigma}$. Lo difícil es notar que $\mathcal{W}$ debe ejecutarse de modo
    tal que, al terminar, el mismo $\mathcal{W}$ está guardado en
    $P\overline{m+1}$. Es decir, 

    \begin{align*}
        \mathcal{W} = \mathcal{R}_{m+1}(\mathcal{W})
    \end{align*}

    lo cual es una auto-referencia que no tiene sentido. No tiene sentido porque
    si
    $\mathcal{W} = q_1 \ldots q_k$ entonces (omitiendo el $SKIP$ inicial)

\begin{align*}
     q_1\ldots q_k = (P\overline{m+1} \leftarrow
     P\overline{m+1}.q_1) \ldots (P\overline{m+1} \leftarrow
     P\overline{m+1}.q_k)
\end{align*}

Es decir que $q_1 = P\overline{m+1} \leftarrow P\overline{m+1}.\left(
P\overline{m+1} \leftarrow P\overline{m+1}.\left( \ldots \right)  \right) $.

La pregunta es: ¿Hay otra forma de dar con un programa $\mathcal{W} \in
Pro^{\Sigma}$ tal que $\mathcal{W}$ termina consigo mismo en $P\overline{m+1}$?
¿Y qué si partimos $\mathcal{W}$? Sabemos que $\mathcal{W} = \mathcal{W}_1
\mathcal{W}_2$ con $\mathcal{W}_1, \mathcal{W}_2 \in Pro^{\Sigma}$. ¿Podemos
hacer que $\mathcal{W}_1$ guarde en $P\overline{m+1}$ el contenido de $\mathcal{W}_2$ y vice-versa? Así
evitaríamos las auto-referencias complicadas.

O sea, podríamos hacer

\begin{align*}
    \mathcal{W}_2 = R_{m+1}(\mathcal{W}_1) \\ 
    \mathcal{W}_1 = R_{m+2}(\mathcal{W}_2)
\end{align*}

y luego concatenar a $\mathcal{W}_2 \mathcal{W}_1$ un programa que "pegue" los
dos programas. Pero acá otra vez hay circularidad!! Porque con esta definición 

\begin{align*}
    \mathcal{W}_2 = R_{m+1} \left( R_{m+2}(\mathcal{W}_2) \right) 
\end{align*}

Se me hace que esta idea de partir $\mathcal{W}$ y hacer $\mathcal{W}_2 =
R_{m+1}(\mathcal{W}_1)$ es un buen comienzo; pero no sé si se puede evitar la
circularidad anterior. ¿Tal vez hay que usar $S_{n,
l, m, k}$ de alguna forma? 
